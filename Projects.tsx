  ...
  const projects = [
    {
      ...
      image: "https://img.sanishtech.com/u/c34f3ccc29385356f2d4b4b15761c7f4.png",
      caseStudy: {
        projectDescription: "CosmicPulse AI is an advanced intelligent radio signal monitoring and visualization system designed to detect, analyze, and display radio frequency signals in real time. The project focuses on improving signal observation accuracy using intelligent detection techniques and modern visualization methods.",
    ...
    },
    {
      ...
      image: "https://img.sanishtech.com/u/7bcb6a55a0d0b00d58bc5bc14f086660.png",
      caseStudy: {
        ...
        {projects.map((project, index) => (
          ...
          >
            ...
            <div className="p-8 flex flex-col flex-grow items-start text-left">
              ...
              <p className="text-sm italic text-gray-400 mt-1">{project.subtitle}</p>
              ...
              {/* 🟢 BUTTONS SECTION - Updated with Live Demo */}
      ...
      {selectedProject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-xl p-5">
          <div className="relative bg-[#0a0a0a]/90 border border-white/10 rounded-[2rem] p-10 max-w-4xl w-full max-h-[85vh] overflow-y-auto shadow-2xl">
            <button onClick={() => setSelectedProject(null)} className="absolute top-6 right-6 text-3xl text-gray-500 hover:text-white transition-colors">✕</button>
            <h2 className="text-4xl font-bold text-cyan-400">{selectedProject.title}</h2>
            <p className="mt-2 text-gray-400 italic mb-8">{selectedProject.subtitle}</p>
            <div className="space-y-10">
              <section>
                <h3 className="text-2xl font-bold border-l-4 border-cyan-500 pl-4">Project Overview</h3>
                <p className="mt-4 text-gray-300 leading-relaxed">{selectedProject.caseStudy.projectDescription}</p>
              </section>
              <section>
                <h3 className="text-2xl font-bold border-l-4 border-cyan-500 pl-4">Main Features</h3>
                <ul className="list-disc ml-6 mt-4 space-y-2 text-gray-300">
                  {selectedProject.caseStudy.features.map((f: string, i: number) => <li key={i}>{f}</li>)}
                </ul>
              </section>
              <section>
                <h3 className="text-2xl font-bold border-l-4 border-cyan-500 pl-4">Working Process</h3>
                <ul className="list-decimal ml-6 mt-4 space-y-2 text-gray-300">
                  {selectedProject.caseStudy.workingProcess.map((w: string, i: number) => <li key={i}>{w}</li>)}
                </ul>
              </section>
              <section>
                <h3 className="text-2xl font-bold border-l-4 border-cyan-500 pl-4">Technical Challenges</h3>
                <div className="grid md:grid-cols-2 gap-6 mt-4 text-gray-300">
                  <div>
                    <h4 className="font-bold text-red-400 mb-2">Challenges:</h4>
                    <ul className="list-disc ml-5 space-y-1">
                      {selectedProject.caseStudy.challenges.map((c: string, i: number) => <li key={i}>{c}</li>)}
                    </ul>
                  </div>
                  <div>
                    <h4 className="font-bold text-green-400 mb-2">Solutions:</h4>
                    <ul className="list-disc ml-5 space-y-1">
                      {selectedProject.caseStudy.solutions.map((s: string, i: number) => <li key={i}>{s}</li>)}
                    </ul>
                  </div>
                </div>
              </section>
              <section>
                <h3 className="text-2xl font-bold border-l-4 border-cyan-400 pl-4">Conclusion</h3>
                <p className="mt-4 text-gray-300 italic">{selectedProject.caseStudy.conclusion}</p>
              </section>
            </div>
            <button onClick={() => setSelectedProject(null)} className="mt-12 px-10 py-3 bg-white/10 hover:bg-white/20 text-white font-bold rounded-xl border border-white/10 transition-all">Close</button>
          </div>
        </div>
      )}
    </section>
